...
| LINEログイン Channel ID | `2010403811`（`LINE_CHANNEL_ID` 用） |
| Messaging API Channel ID | `2010403797`（push トークン用） |
...
2. **プロバイダー** 配下に次の **2チャネル** があることを確認する
   - **LINEログインチャネル** — LIFF・ログイン（`LINE_CHANNEL_ID` はこちら）
   - **Messaging API チャネル** — 公式 LINE・push（`LINE_CHANNEL_ACCESS_TOKEN` はこちら）
3. 同じプロバイダー内なら、LIFF で取得した `userId` と push 先の `userId` は **同じ値**

### A-2. LIFF アプリ作成（LINEログインチャネル）
...
| Secret 名 | 値 | 備考 |
|-------------|-----|------|
| `LINE_CHANNEL_ID` | **LINEログインチャネル**の Channel ID | LIFF と同じチャネル（ID トークン検証用） |
